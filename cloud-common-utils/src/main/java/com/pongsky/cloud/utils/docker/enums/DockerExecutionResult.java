...
public enum DockerExecutionResult {
     ...
     * 创建服务
     */
    CREATE_SERVICE_RESULT("Creating service {0}_{0}", null, null),

    /**
     * 删除服务
     */
    REMOVE_SERVICE_RESULT("Removing service {0}_{0}", null, null),

    /**
     * 更新服务
     */
    UPDATE_SERVICE_RESULT(null, "{0}_{0}", "verify: Service converged");
    ...
    public static void validationUpdateService(String cmdResult, Script script) {
        ...
        if (!cmdResult.startsWith(start) || !cmdResult.endsWith(UPDATE_SERVICE_RESULT.getEnd())) {
